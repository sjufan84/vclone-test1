...
def get_model(sid):
    if sid == "Joel":
        model = "./weights/joel11320.pth"
    elif sid == "Jenny":
        model = "./weights/jenny3450.pth"
    else:
        model = None
    return model
...
def get_index(sid0):
    global index
    if sid0 == "Joel":
        index = "./logs/joel_index.index"
    elif sid0 == "Jenny":
        index = "./logs/jenny.index"
    else:
        index = ""
...
def get_indexes():
    global indexes_list
    indexes_list=["./logs/jenny.index", "./logs/joel_index.index"]
    return indexes_list
    ...
    with gr.Tabs():
        with gr.TabItem("Inference"):
            ...
            with gr.Row():
                global sid0
                global model
                sid0 = gr.Dropdown(label="1.Choose your Model.", choices=sorted(name_choices), value=name_choices[0])
                refresh_button = gr.Button("Refresh", variant="primary")
                model = get_model(sid0)
                if model:
                    get_vc(model)
                ...
                with gr.Column():
                    with gr.Accordion("Index Settings", open=False):
                        file_index1 = gr.Dropdown(
                            ...
                            choices=get_indexes(),
                            value=get_index(sid0),
                            interactive=True,
                            )
                        sid0.change(fn=get_index, inputs=[sid0],outputs=[file_index1])
